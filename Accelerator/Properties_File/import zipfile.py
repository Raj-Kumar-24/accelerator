...
def entity_export(params):
    # Code for consistency check
    Entity.main_entity(params)
    #dict_to_str = json.dumps(params)
    #subprocess.run(["python", "C:/Accelerator/Entity.py", dict_to_str])
    #result = subprocess.run(["python", "C:/Accelerator/Entity.py", dict_to_str], capture_output=True, text=True)
    
    # "C:/Accelerator/Reltio_app.py"
    ...
    if config_file is not None:
        ...
        if st.button("Run Job"):
            if job_type == "Entity Export":
                ...
                relation_export(config_file)
            elif job_type == "Count Check":
                #user_input = st.text_input("Please enter the delimiter and country code(s) separated by commas:")
                st.write('Job Executed Successfully...')
...
#     job_type = st.selectbox(
#         "Select Job Type",
#         ["Entity Export", "Relation Export", "Merge Extract"]
#     )
...
#         config_params = json.load(config_file)
#         # Run the selected job
#         if st.button("Run Job", disabled=st.session_state.button_disabled):
...
#             elif job_type == "Relation Export":
#                 relation_export(config_file)
#             elif job_type == "Merge Extract":
#                 merge_extract(config_file)
...
def main_cnt(config_params,user_input):
    ...
    if delimiter_country_code == ['']:
        ...
        delimiter = delimiter_country_code
...
def count_crosswalk_values(df, user_entities, user_sources, user_country_code=None, delimiter=':'):
    # Ensure user_entities is a list and convert to lowercase for case insensitivity
...
def process_files(properties, user_country_code, delimiter):
    ...
    if df_list:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        count_analysis_report = os.path.join(output_directory, f"Count_Analysis_Report_{timestamp}.xlsx")
        os.makedirs(os.path.dirname(count_analysis_report), exist_ok=True)
